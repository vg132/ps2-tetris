...
void CField::ClearBlock(u16 block[][4])
{
	for(u8 x=0;x<4;x++)
	{
		for(u8 y=0;y<4;y++)
...
void CField::NewBlock(u16 block[][4],u8 type, bool next)
{
	if(next)
		iNextColor=type-20;
	ClearBlock(block);
...
bool CField::CanMove(u8 direction)
{
	ClearPos();
	for(u8 y=0;y<4;y++)
	{
...
void CField::Rotate(u8 direction)
{
	if(iStatus!=STATUS_NORMAL)
		return;
	ClearPos();
...
u16 CField::Randnum(u16 limit)
{
	return(rand()%limit);
}

u8 CField::CheckFullRow(void)
{
	bool fullrow=false;
	u8 rowCount=0;
...
void CField::SetRemoveFullRow(u8 row)
{
	for(u8 x=0;x<10;x++)
		iField[x][row]=MAKELONG(iField[x][row],BLOCK_REMOVE);
}

void CField::RemoveFullRow(u8 row)
{
	s8 y(0),x(0);
	for(y=row-1;y>=0;y--)
	{
...
void CField::SetStartHeight(u8 height)
{
	u8 block=0;
	for(u8 y=HEIGHT-1;y>(HEIGHT-(height+1));y--)
	{
...
bool CField::AddSolidBlocks(u8 nr)
{
	u8 y(0), x(0);
	iY-=nr;
	if(iY<0)
...
u8 CField::RemoveSolidBlocks(u8 nr)
{
	u8 y(0),x(0);
...
void CField::Flip()
{
	...
	{
		u8 tmp(0),bottom(HEIGHT-1),x(0);
		u8 stop=(HEIGHT-found)/2+found;
