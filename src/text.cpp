...
void CText::PrintText(const char *text, u16 x, u16 y, u32 color, Ito::GifPipe *gif, u8 font, u8 align)
{
	int margin(0), size(8), newX(0);
	...
	switch(align)
	{
		case ALIGN_LEFT:
			...
			break;
		case ALIGN_RIGHT:
			newX=x-(size*strlen(text));
			break;
		case ALIGN_CENTER:
			newX=x-((size*strlen(text))/2);
			break;
	}

	while(*text)
	{
		PrintChar(*text++,newX,y,color,gif,font);
		newX+=size;
	}
}

void CText::PrintChar(const char ch, u16 x, u16 y, u32 color, Ito::GifPipe *gif, u8 font)
{
	u16 i, j,cc;

	if(font==SMALL_FONT)
	{
		u8 *pc=&m_SmallFont[(ch-32)*8];
		for(i=0;i<8;i++)
		{
			cc=*pc++;
			for(j=0;j<8;j++)
	...
	{
		u16 *pc=&m_LargeFont[((ch-32)*16)+2048];
